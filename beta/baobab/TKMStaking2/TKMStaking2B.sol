...
contract TKMStaking2B is Pausable, AccessControl {
    ...
    // stakeId (9 ~ 12: 3km, 13 ~ 16: dtc)
    uint256[8] public stakeIdList = [9, 10, 11, 12, 13, 14, 15, 16];
    ...
    function setStaking(
        ...
        if(0 != stakings[_stakeId].startTime) {
            require(_endTime > stakings[_stakeId].startTime, "[TKMStaking2B][setStaking]: _endTime is under _startTime");
            require(_endTime > block.timestamp, "[TKMStaking2B][setStaking]: _endTime is under now");
    ...
    function stake(uint256 _stakeId, uint256[] memory _ids) public whenNotPaused {
        // stake id 체크
        require(true == stakeIds[_stakeId], "[TKMStaking2B][stake]: invalid stake id");

        // 기간 체크
        require(stakings[_stakeId].startTime <= block.timestamp, "[TKMStaking2B][staking]: staking is not started");
        require(stakings[_stakeId].endTime >= block.timestamp, "[TKMStaking2B][staking]: staking is ended");

        // nft 수량 체크
        require(stakings[_stakeId].nftAmount == _ids.length, "[TKMStaking2B][staking]: not equal nftAmount");

        // 오너 체크
        for (uint256 i = 0; i < stakings[_stakeId].nftAmount; i++) {
            require(TKMNft.ownerOf(_ids[i]) == msg.sender, "[TKMStaking2B][staking]: owner invalid");
        }
        ...
        // stakedInfo 갱신
        stakedInfoList[_stakeId - gapIndex].nftCount += 3;
    ...
    function withdraw(uint256 _stakingIdx) public whenNotPaused {
        // 스테이킹 존재 및 지갑 주소 체크
        require(progressing[_stakingIdx].user != address(0), "[TKMStaking2B][withdraw]: not exist staking data");
        require(progressing[_stakingIdx].user == msg.sender, "[TKMStaking2B][withdraw]: not Owner");

        // stakedInfo 갱신
        stakedInfoList[progressing[_stakingIdx].stakeId - gapIndex].nftCount -= 3;
    ...
    function harvest(uint256 _stakingIdx) public whenNotPaused {
        // 스테이킹 존재 및 지갑 주소 체크
        require(progressing[_stakingIdx].user != address(0), "[TKMStaking2B][harvest]: not exist staking data");
        require(progressing[_stakingIdx].user == msg.sender, "[TKMStaking2B][harvest]: not Owner");
