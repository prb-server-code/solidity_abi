...
contract TKMStaking2 is Pausable, AccessControl {
    ...
    function setStaking(
        ...
        if(0 != stakings[_stakeId].startTime) {
            require(_endTime > stakings[_stakeId].startTime, "[TKMStaking2][setStaking]: _endTime is under _startTime");
            require(_endTime > block.timestamp, "[TKMStaking2][setStaking]: _endTime is under now");
    ...
    function stake(uint256 _stakeId, uint256[] memory _ids) public whenNotPaused {
        // stake id 체크
        require(true == stakeIds[_stakeId], "[TKMStaking2][stake]: invalid stake id");

        // 기간 체크
        require(stakings[_stakeId].startTime <= block.timestamp, "[TKMStaking2][staking]: staking is not started");
        require(stakings[_stakeId].endTime >= block.timestamp, "[TKMStaking2][staking]: staking is ended");

        // nft 수량 체크
        require(stakings[_stakeId].nftAmount == _ids.length, "[TKMStaking2][staking]: not equal nftAmount");

        // 오너 체크
        for (uint256 i = 0; i < stakings[_stakeId].nftAmount; i++) {
            require(TKMNft.ownerOf(_ids[i]) == msg.sender, "[TKMStaking2][staking]: owner invalid");
        }
    ...
    function withdraw(uint256 _stakingIdx) public whenNotPaused {
        // 회수 가능 날짜 체크
        require(block.timestamp > stakings[progressing[_stakingIdx].stakeId].withdrawTime, "[TKMStaking2][withdraw]: withdraw time is still left");

        // 스테이킹 존재 및 지갑 주소 체크
        require(progressing[_stakingIdx].user != address(0), "[TKMStaking2][withdraw]: not exist staking data");
        require(progressing[_stakingIdx].user == msg.sender, "[TKMStaking2][withdraw]: not Owner");
    ...
    function harvest(uint256 _stakingIdx) public whenNotPaused {
        // 스테이킹 존재 및 지갑 주소 체크
        require(progressing[_stakingIdx].user != address(0), "[TKMStaking2][harvest]: not exist staking data");
        require(progressing[_stakingIdx].user == msg.sender, "[TKMStaking2][harvest]: not Owner");
