...
contract TKMStaking4 is Pausable, Ownable {
    ...
    function setStaking(
        ...
        if(0 != stakings[_stakeId].startTime) {
            require(_endTime > stakings[_stakeId].startTime, "[TKMStaking4][setStaking]: _endTime is under _startTime");
            require(_endTime > block.timestamp, "[TKMStaking4][setStaking]: _endTime is under now");
    ...
    function stake(uint256 _stakeId, uint256 _amount) public whenNotPaused {
        // stake id 체크
        require(true == stakeIds[_stakeId], "[TKMStaking4][stake]: invalid stake id");

        // stake 상태 체크
        require(false == stakeJoin[msg.sender][_stakeId], "[TKMStaking4][stake]: already staked");
        stakeJoin[msg.sender][_stakeId] = true;

        // 기간 체크
        if(0 != stakings[_stakeId].endTime) {
            require(stakings[_stakeId].endTime >= block.timestamp, "[TKMStaking4][stake]: staking is ended");
        }
        require(stakings[_stakeId].startTime <= block.timestamp, "[TKMStaking4][stake]: staking is not started");

        // minTokenAmount 체크
        require(stakings[_stakeId].minTokenAmount <= _amount, "[TKMStaking4][stake]: Staking amount not enough");

        // perTokenAmount 체크
        require(_amount % stakings[_stakeId].perTokenAmount == 0, "[TKMStaking4][stake]: Staking amount invalid");
    ...
    function withdraw(uint256 _stakingIdx) public whenNotPaused {
        // 스테이킹 존재 및 지갑 주소 체크
        require(progressing[_stakingIdx].user != address(0), "[TKMStaking4][withdraw]: not exist staking data");
        require(progressing[_stakingIdx].user == msg.sender, "[TKMStaking4][withdraw]: not Owner");
    ...
    function harvest(uint256 _stakingIdx) public whenNotPaused {
        // 스테이킹 존재 및 지갑 주소 체크
        require(progressing[_stakingIdx].user != address(0), "[TKMStaking4][harvest]: not exist staking data");
        require(progressing[_stakingIdx].user == msg.sender, "[TKMStaking4][harvest]: not Owner");
