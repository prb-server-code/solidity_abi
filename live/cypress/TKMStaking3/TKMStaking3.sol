...
contract TKMStaking3 is Pausable, Ownable {
    ...
    function setStaking(
        ...
        if(0 != stakings[_stakeId].startTime) {
            require(_endTime > stakings[_stakeId].startTime, "[TKMStaking3][setStaking]: _endTime is under _startTime");
            require(_endTime > block.timestamp, "[TKMStaking3][setStaking]: _endTime is under now");
    ...
    function stake(uint256 _stakeId, uint256 _amount) public whenNotPaused {
        // stake id 체크
        require(true == stakeIds[_stakeId], "[TKMStaking3][stake]: invalid stake id");

        // 기간 체크
        if(0 != stakings[_stakeId].endTime) {
            require(stakings[_stakeId].endTime >= block.timestamp, "[TKMStaking3][stake]: staking is ended");
        }
        require(stakings[_stakeId].startTime <= block.timestamp, "[TKMStaking3][stake]: staking is not started");

        // minTokenAmount 체크
        require(stakings[_stakeId].minTokenAmount <= _amount, "[TKMStaking3][stake]: Staking amount not enough");

        // maxTokenAmount 체크
        stakings[_stakeId].curTokenAmount += _amount;
        require(stakings[_stakeId].curTokenAmount <= stakings[_stakeId].maxTokenAmount, "[TKMStaking3][stake]: Staking amount overflow");
    ...
    function withdraw(uint256 _stakingIdx) public whenNotPaused {
        // 스테이킹 존재 및 지갑 주소 체크
        require(progressing[_stakingIdx].user != address(0), "[TKMStaking3][withdraw]: not exist staking data");
        require(progressing[_stakingIdx].user == msg.sender, "[TKMStaking3][withdraw]: not Owner");

        // 락 시간 체크
        require(block.timestamp > progressing[_stakingIdx].releaseTime, "[TKMStaking3][withdraw]: remaining time is still left");
